<template>
  <q-expansion-item
    ...
    group="exp"
    :default-opened="index === 0 && !$q.screen.xs"
    popup
  >
    <template v-slot:header>
      <q-item-section class="section-experience-title text-white">
        <q-img
          class="image-miniature"
          :src="experienceLogoMiniature(experience.srcLogo)"
        />
        {{ experience.name }}
        <span
          v-if="$q.screen.xs"
      ...
      <q-item-section class="text-white" v-if="!$q.screen.xs" side>
...
function experienceLogo(logo: string): string {
  return new URL(`/src/assets/experience/${logo}.png`, import.meta.url).href;
}

function experienceLogoMiniature(logo: string): string {
